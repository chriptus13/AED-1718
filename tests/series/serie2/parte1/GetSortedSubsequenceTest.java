...
import static series.serie2.parte1.Iterables.getSortedSubsequence;
...
public class GetSortedSubsequenceTest extends IterablesTest {
    ...
    static final List<Integer> SEquals = unmodifiable(Arrays.asList(5, 5, 5, 5, 5));
    static final List<Integer> SSome = unmodifiable(Arrays.asList(1, 3, 2, 5, 7, 6, 4, 8));

    @Test
    public void getSortedSubsequence_withEmptySequences() {
        assertIterableEmpty(getSortedSubsequence(SEmpty, 5));
    }
    ...
    public void getSortedSubsequence_withEqualsSequence() {
        ...
        for (int i = 0; i < SEquals.size(); i++) {
            ...
            assertIterableEmpty(getSortedSubsequence(subList, 6));
        }
        for (int i = 0; i < SEquals.size(); i++) {
            subList = SEquals.subList(i, SEquals.size());
            assertIterableEquals(subList, getSortedSubsequence(subList, 5));
    ...
    public void getUnSortedSequence_withSomeSequence() {
        List<Integer> someSubsequence = unmodifiable(Arrays.asList(5, 7, 8));
